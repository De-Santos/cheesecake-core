...
public enum TagQuery {
    ...
    DELETE_ALL_MATCHING("DELETE FROM tag_collection_tags WHERE tags_id = ?"),
    ...
    EXIST_MATCHING_TO_DRAFT("""
            SELECT EXISTS(
                ...
                JOIN tags t ON tct.tags_id = t.id
            ...
            """),
    MATCH_TO_DRAFT("""
            INSERT INTO tag_collection_tags (tag_collection_id, tags_id)
            SELECT tc.id, ?
            FROM drafts d
    ...
    SELECT_BY_DRAFT_ID("""
            ...
            FROM tags t
            JOIN tag_collection_tags tct ON t.id = tct.tags_id
            JOIN tag_collection tc ON tct.tag_collection_id = tc.id
            JOIN drafts d ON tc.draft_product_hash = d.hash
            WHERE d.id = ?
            """),
